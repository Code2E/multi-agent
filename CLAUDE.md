...
- [x] `core/schemas.py` — Part VII 모든 Pydantic 모델 + v4 보정 4건
- [x] `agents/base.py` — Agent Protocol + InvocationContext
- [x] 4 에이전트 스켈레톤 (`planner` / `executor` / `advisor` / `evaluator`)
- [x] 코어 11종 Protocol/시그니처 (`orchestrator` / `state` / `termination` / `llm_gateway` / `cassette` / `budget` / `logger` / `checkpoint` / `hooks` / `process_manager` / `port_allocator`)
- [x] `runners/` (Protocol + Playwright stub) / CLI Typer App + 13 서브커맨드 / UI 2종 / 프롬프트 6종 / HTML 리포트 템플릿
- [x] CLAUDE.md 4원칙 행동 가이드라인 (commit `6f00b61`)
- [x] GitHub remote 연결 + `Code2E/multi-agent` `main` 에 push

### 완료 — Phase 2 코어 인프라

- [x] `core/cassette.py` 구현 + 단위 테스트 17건 (commit `2e37c26`)
  - compute_key (sha256, repair_round 포함) / canonicalize (volatile 필드 제외) / redact (Bearer / sk- / key-name) / try_hit / record (NNNNN.{key8}.json + schema_version 검사) / manifest stub
- [x] `core/budget.py` 구현 + 단위 테스트 12건 (commit `4e5ff93`)
  - check_headroom / add / usage_ratio (limit=0 안전) / should_warn (warn_threshold 도달 1회만)
- [x] `core/logger.py` — 스캐폴드 시점에 이미 동작 가능한 형태 (configure / get_logger)

### 다음 단계 (TO DO, 우선순위 순)

- [ ] `core/orchestrator.py` 의 순수 함수 3종: `parse_units_from_plan` / `validate_unit_dag` / `topological_sort` + `truncate_failure_report` + 단위 테스트 (v4 보정 #4 의 실제 구현, Q33/Q39 결정 박기)
- [ ] `core/llm_gateway.py` 구현 — Anthropic provider adapter + 6단계 파이프라인 (cache → budget → call → validate → repair × 2 → record)
- [ ] **Planner 프롬프트 3종 본문 작성** — round 1/2/3 의 system + user 템플릿
- [ ] `agents/planner.py.invoke` 구현 + 단위 테스트 (cassette replay 기반)
...
- [ ] `core/checkpoint.py` 구현 (runs/.global.lock — Q19)
- [ ] `core/hooks.py` discover/dispatch 구현
- [ ] CLI `run` / `doctor` / `init` / `inspect` 본문 구현
- [ ] pyright `strict` 격상 + 모든 type 경고 해소 + golden snapshot 인프라

### 결정 / 확인 필요 (open)

- [ ] LLM provider config: `claude-sonnet-4-6` 디폴트 — `config/default.yaml` 에 박혀있음. 변경 시 cassette 키 invalidate.
- [ ] `paran/code2e-agent/reference/` 가 본 저장소에서 (현재) 상대경로로 참조됨. 저장소 분리가 더 진행되면 reference 사본을 본 repo 에 둘지 / symlink / 외부 링크로 둘지 결정.

---

## 변경 로그

- **2026-05-05** — 저장소 신설. v4 plan 기반 스캐폴드 완료. 검증 4종 통과. §19.2 / §19.3 결정 + v4 보정 4건 반영. CLAUDE.md 4원칙 가이드라인 추가.
- **2026-05-05** — `core/cassette.py` 구현 + 17 tests (compute_key / canonicalize / redact / try_hit / record).
- **2026-05-05** — `core/budget.py` 구현 + 12 tests (check_headroom / add / usage_ratio / should_warn).
- **2026-05-05** — GitHub `Code2E/multi-agent` 연결, `main` 에 push (5 commits). 브랜치 정책 갱신: 본 repo 는 `main` 직접 작업 OK.
