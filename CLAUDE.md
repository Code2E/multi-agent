...
**누적**: 45 commits / 342 tests / 검증 4종 모두 PASS / 약 96% 완성도.
**상태**: v1 critical path + 핵심 운영 도구 + **대화형 인터페이스 (Chat + TUI)** 동작.
4 task 종류 end-to-end 검증 (Hello World / Calculator / TODO / Health check /
SaaS Landing / TODO UI).
...
### 대화형 인터페이스 (2026-05-18 추가)

batch `code2e run` 외 두 가지 대화형 모드. 둘 다 같은 EventEmitter
backend 공유 — 산출 일관성.

```bash
python -m code2e chat     # 로컬 웹 UI (브라우저 자동 open, localhost:9876)
python -m code2e tui      # Rich Live 터미널 UI (SSH / 헤드리스용)
```

`code2e chat` 의 4 컬럼 레이아웃: History sidebar / Conversation /
Pipeline Monitor / Result Preview (산출 앱 iframe + Source 탭).
follow-up task / cancel / toast 알림 지원. chat 모드에선 Teardown 을
skip 해 산출 앱이 iframe preview 로 계속 살아있음 (다음 task 시작 시
자동 정리).
...
- **Interactive HITL (Phase 별 사용자 승인 / 자연어 follow-up 으로 plan 수정)**
  — `code2e chat` / `code2e tui` 는 **모니터링용** 대화형 인터페이스. plan
  중간 개입은 v4 결정성과 충돌하므로 별도 ADR 필요.
...
- **2026-05-18 (오후)** — 대화형 인터페이스 도입: `code2e chat` (로컬 웹 v0→v1→v2, SSE + iframe + history + cancel + source) + `code2e tui` (Rich Live 터미널). 공통 `core/event_emitter.py` (Orchestrator hook, batch 호환 noop). 4 commits + Phase 3 BudgetExceededError catch fix (영역 2 보완).
